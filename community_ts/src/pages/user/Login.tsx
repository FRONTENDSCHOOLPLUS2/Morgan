import { useCallback, useRef, useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import Submit from "@components/Submit";
import login from "../../api/login";
import useProfileStore from "../../zustand/useProfile";

const Login = () => {
    const navigator = useNavigate();
    const { profile, updateProfile } = useProfileStore();
    const inputRefMap = useRef<Map<string, HTMLInputElement>>(new Map());
    const [isWarning, setIsWarning] = useState<boolean>(false);
    const [input, setInput] = useState({
        email:"morgan@market.com",
        password: "11111111"
    });
    const [warningMessage, setWarningMessage] = useState("");

    const setInputRef = useCallback((node: HTMLInputElement) => {
        if (node !== null) inputRefMap.current.set(node.name, node);
    }, []);

    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        const formData = new FormData(e.currentTarget);
        for (const [name, value] of formData) {
            if (value === "") return inputRefMap.current.get(name)?.focus();
        }
        const info: { [k:string]: FormDataEntryValue } = Object.fromEntries(formData.entries());
        // console.log(info);

        try {
            const res = await login(info);
            // console.log(res);
            // 아이디 중복 체크를 통해서, 로그인 실패 문구 세분화하기
            
            if (res.ok === 1) {
                console.log("Login Success!!!")
                setIsWarning(false);
                updateProfile(res.item);
                // console.log(profile);
                navigator('/');
            } else {
                setIsWarning(true);
                setWarningMessage(
                    res.errors ? res.errors[0].msg : res.message
                );
                console.log(res);
                console.log("Not Access");
            }

        } catch (err) {
            console.error(err);
        }

    }

    const onChangeHandler = (e:  React.ChangeEvent<HTMLInputElement>) => {
        if(e.target.name === "email") {
            setInput({
                email: e.target.value,
                password: input.password,
            })
        } else if (e.target.name === "password") {
            setInput({
                email: input.email,
                password: e.target.value,
            });
        }
    }

    console.log(profile);

    return (
    <main className="min-w-80 flex-grow flex items-center justify-center">
        <div className="p-8 border border-gray-200 rounded-lg w-full max-w-md dark:bg-gray-600 dark:border-0">
        <div className="text-center py-4">
            <h2 className="text-2xl font-semibold text-gray-700 dark:text-gray-200">로그인</h2>
        </div>

        <form 
            onSubmit={onSubmit}
        >
            <div className="mb-4">
            <label className="block text-gray-700 dark:text-gray-200 mb-2" htmlFor="email">이메일</label>
            <input
                ref={setInputRef}
                id="email"
                type="email"
                placeholder="이메일을 입력하세요"
                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-orange-400 dark:bg-gray-700"
                value={input.email}
                onChange={onChangeHandler}
                name="email"
            />
            {/* 입력값 검증 에러 출력 */}
            { isWarning 
            ? (<p className="ml-2 mt-1 text-sm text-red-500 dark:text-red-400">{warningMessage}</p>)
            : null}

            </div>
            <div className="mb-4">
            <label className="block text-gray-700 dark:text-gray-200 mb-2" htmlFor="password">비밀번호</label>
            <input
                ref={setInputRef}
                id="password"
                type="password"
                placeholder="비밀번호를 입력하세요"
                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-orange-400 dark:bg-gray-700"
                value={input.password}
                onChange={onChangeHandler}
                name="password"
            />
            {/* 입력값 검증 에러 출력 */}
            {isWarning 
            ? (<p className="ml-2 mt-1 text-sm text-red-500 dark:text-red-400">{warningMessage}</p>)
            : null}

            <Link to="#" className="block mt-6 ml-auto text-gray-500 text-sm dark:text-gray-300 hover:underline">비밀번호를 잊으셨나요?</Link>
            </div>
            <div className="mt-10 flex justify-center items-center">
            <Submit>로그인</Submit>
            <Link to="/user/signup" className="ml-8 text-gray-800 hover:underline">회원가입</Link>
            </div>
        </form>
        </div>
    </main>
    );
}

export default Login;